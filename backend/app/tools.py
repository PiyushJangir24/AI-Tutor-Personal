...
async def call_note_maker(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        async with httpx.AsyncClient(base_url=settings.tool_base_url, timeout=10) as client:
            resp = await client.post("/note_maker", json=req.model_dump())
...
async def call_flashcard_generator(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        async with httpx.AsyncClient(base_url=settings.tool_base_url, timeout=10) as client:
            resp = await client.post("/flashcard_generator", json=req.model_dump())
            resp.raise_for_status()
            data = resp.json()
            return data.get("data", data)
    except Exception:
...
async def call_concept_explainer(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        async with httpx.AsyncClient(base_url=settings.tool_base_url, timeout=10) as client:
            resp = await client.post("/concept_explainer", json=req.model_dump())
            resp.raise_for_status()
            data = resp.json()
            return data.get("data", data)
    except Exception:
